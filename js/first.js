...
require(['ojs/ojcore', 'knockout', 'jquery', 'ojs/ojknockout', 
  ...
  function (oj, ko, $) { // this callback gets executed when all required modules are loaded
      ...
      {
           var self =this;
          self.uname=ko.observable(oj.Translations.getTranslatedString('developer'));
          ...
          self.selectionDone=function(event)
          {
              let listOfFiles=event.detail.files;
              
              self.selectedFiles([]);
              for(let i=0;i<listOfFiles.length;i++)
              {
                  let name=listOfFiles[i].name;
                  self.selectedFiles.push(name);
              }
          }
      }
